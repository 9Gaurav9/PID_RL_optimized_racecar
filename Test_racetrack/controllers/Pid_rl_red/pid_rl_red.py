...
LAP_REENTRY_DIST = 3  # Must return within this to count lap
...
camera = driver.getDevice("red_camera")
...
# === PPO Model ===
# ppo_model = PPO.load("../../Model/saved_model/ppo_model")
model_path = os.path.join(os.path.dirname(__file__), "..", "..", "..", "Model", "saved_model", "ppo_model.zip")
...
lap_zone_state = "unknown"
lap_locked_until_exit = False
lap_unlock_time = 0


def check_lap(pos, heading):
    global lap_started, lap_completed, best_lap_time, lap_start_time
    global last_lap_time, lap_cooldown, lap_zone_state, lap_locked_until_exit, lap_unlock_time
    ...
    correct_heading = abs(heading) < 0.3 # 0.6

    # Cooling off after lap completion
    if lap_cooldown > 0:
        lap_cooldown -= 1
        return

    # Unlock lap only after time threshold and sufficient distance
    if dist > LAP_EXIT_DIST and current_time > lap_unlock_time:
        lap_zone_state = "outside"
        lap_locked_until_exit = False
        if not lap_started:
            lap_started = True
            lap_completed = False
    ...
    # Prevent repeated triggering if still inside zone
    elif dist < LAP_REENTRY_DIST:
        if lap_locked_until_exit:
            return  # don't allow reentry without full unlock
        lap_zone_state = "inside"
        lap_elapsed = current_time - lap_start_time

        if lap_started and not lap_completed and correct_heading and lap_elapsed > MIN_LAP_DURATION:
            last_lap_time = lap_elapsed
            lap_start_time = current_time
            lap_completed = True
            lap_started = False
            lap_locked_until_exit = True
            lap_cooldown = LAP_COOLDOWN_FRAMES
            lap_unlock_time = current_time + 5.0  # must wait 5s before new unlock
...
while driver.step() != -1:
    ...
    pos = gps.getValues()
    # Get opponent position from customData
    
    # === Access to customData
    custom_data = driver.getCustomData()
    try:
        opp_x, opp_z = map(float, custom_data.split(","))
    except ValueError:
        opp_x, opp_z = -999, -999  # fallback if no data
    ...
    if START_POS is None:
        ...
        # Relaxed conditions or manual override
        stationary_counter += 1
        if stationary_counter >= 5:  # ← force lock after small delay
            START_POS = (pos[0], pos[2])
            driver.setCruisingSpeed(40)
            print(f"[INFO] START_POS forcibly set to {START_POS}")
        continue
    ...
    throttle = 5.0 + raw_throttle * 20.0  # Initialize throttle BEFORE using it
    ...
    # Boost if we're close behind and aligned laterally
    # if 0.0 < rel_dx < 3.0 and abs(rel_dz) < 1.0:
        # throttle += 2.0  # Speed up to overtake
    
    # Nudge laterally to overtake
    # if 0.0 < rel_dx < 3.0:
        # steer += 0.1 if pos[2] < opp_z else -0.1
    ...
    steer = 0.8 * steer_pid + 0.2 * steer_rl
    ...
    # === Base PPO throttle ===
    # throttle = 5.0 + raw_throttle * 20.0  # base range: [5,30]
    
    
    # === Scale throttle down based on steering sharpness ===
    steer_magnitude = abs(steer)
    
    # Soft penalty (never below 0.8 unless sharp)
    steer_penalty = max(0.8, 1.0 - (steer_magnitude / 1.5))
    throttle *= steer_penalty
    
    # Extra penalty on sharp turns
    if steer_magnitude > 0.5:
        throttle *= 0.9
    
     
        # === Final throttle logic with motion check ===
    if raw_throttle > 0.05:
        if throttle < 15.0:
            print(f" [BOOST] Low throttle ({throttle:.2f}) with steer {steer:.2f} → Forcing 15.0")
            throttle = 15.0
    else:
        if steer_magnitude < 0.1 and speed < 1.0:
            # Allow stopping only if nearly straight and almost stationary
            throttle = 0.0
        else:
            if speed < 1.0:
                print(f" [OVERRIDE] PPO throttle=0 while moving/turning → forcing 15.0")
            throttle = 20.0
    
     # === Enhanced Overtaking Logic with Recovery ===
    distance_to_opp = math.sqrt(rel_dx**2 + rel_dz**2)
    aligned_lane = abs(rel_dz) < 1.0
    straight_heading = abs(orientation[2]) < 0.35
    not_turning_sharply = abs(steer) < 0.5
    min_overtake_speed = 5.0
    
    if not hasattr(driver, 'overtake_state'):
        driver.overtake_state = "idle"
    
    # === Start Overtake ===
    if driver.overtake_state == "idle" and 1.5 < distance_to_opp < 5.0 and aligned_lane and straight_heading and not_turning_sharply:
        if speed > min_overtake_speed:
            throttle += 6.0
            steer += -0.2 if pos[2] < opp_z else 0.2
            driver.overtake_state = "overtaking"
            print("[OVERTAKE] Engaged")
    
    # === Continue Overtake: Maintain line
    elif driver.overtake_state == "overtaking":
        if rel_dx > 5.0:  # Passed the car
            driver.overtake_state = "recenter"
            print("[OVERTAKE] Passed → Start Re-centering")
    
    # === Re-center to lane
    elif driver.overtake_state == "recenter":
        if abs(pos[2]) > 0.1:
            correction = -0.08 if pos[2] > 0 else 0.08
            steer += correction
            print(f"[RECENTER] pos[2]={pos[2]:.2f} → steer += {correction:.2f}")
        else:
            driver.overtake_state = "idle"
            print("[OVERTAKE] Done → Back to lane")
    
    # === Final Steer Clipping ===
    steer = float(np.clip(steer, -0.6, 0.6))
